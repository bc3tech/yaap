...
public abstract class Expert : Grpc.Expert.Expert.ExpertBase, IYaapClient, IHostedService
{
    ...
    protected Grpc.YaapServer.YaapServer.YaapServerClient? YaapServer { get; }
    ...
    protected Expert(IConfiguration appConfig, ILoggerFactory loggerFactory, IHttpClientFactory httpClientFactory, Kernel sk, PromptExecutionSettings promptSettings, Grpc.YaapServer.YaapServer.YaapServerClient? yaapServer = null)
    {
        ...
        (
            ...
            yaapServer is null ? null : new(Throws.IfNullOrWhiteSpace(appConfig["URLS"])!.Split(';', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)![0])
        );

        _log = loggerFactory.CreateLogger(this.Detail.Name);

        this.YaapServer = yaapServer;
    }
    ...
    public virtual async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        if (this.YaapServer is not null)
        {
            _log.IntroducingMyself();
            await this.YaapServer.HelloAsync(this.Detail, cancellationToken: cancellationToken);
        }
    ...
    public override async Task<AnswerResponse> GetAnswer(AnswerRequest request, ServerCallContext context)
    {
        ...
        var answer = await AIHelpers.GetAnswerAsync(_kernel, _promptSettings, request.ChatHistory, context.CancellationToken, _log).ConfigureAwait(false);
    ...
    public virtual async Task SayHelloAsync(CancellationToken cancellationToken) => await this.YaapServer!.HelloAsync(this.Detail, cancellationToken: cancellationToken);

    public virtual async Task SayGoodbyeAsync(CancellationToken cancellationToken) => await this.YaapServer!.GoodbyeAsync(this.Detail, cancellationToken: cancellationToken);
