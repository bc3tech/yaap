...
public static partial class ClientUtils
{
    ...
    {
        Match match = FilenameHasPathRegex().Match(filename);
    ...
    /// </summary>
    public static readonly Regex JsonRegex = JsonRegexGen();
    ...
    public static bool IsJsonMime(string mime) => !string.IsNullOrWhiteSpace(mime) && (JsonRegex.IsMatch(mime) || mime.Equals("application/json-patch+json"));

    [GeneratedRegex(@".*[/\\](.*)$")]
    private static partial Regex FilenameHasPathRegex();

    [GeneratedRegex("(?i)^(application/json|[^;/ \t]+/[^;/ \t]+[+]json)[ \t]*(;.*)?$", RegexOptions.None, "en-US")]
    private static partial Regex JsonRegexGen();
}
